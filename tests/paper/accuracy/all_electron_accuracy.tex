...
\section{All-electron Kohn--Sham accuracy}
...
We also assess the accuracy of SPARC-atomSFE for all-electron RPA-OEP calculations, considering the atoms $\mathrm{He}$, $\mathrm{Be}$, and $\mathrm{Ne}$. We employ a polynomial mesh with $s=2$, $R_{\max}=13$~bohr, $p_1=15$, $p_3=4$, and $q=55$; $N_{\mathrm{fe}}$, $l_{\max}$, and $N_\omega$ are $(20,18,30)$, $(30,18,40)$, and $(30,40,60)$ for $\mathrm{He}$, $\mathrm{Be}$, and $\mathrm{Ne}$, respectively, which converges energies and eigenvalues to within $5\times10^{-4}$~Ha. The results agree with the literature to within $10^{-3}$~Ha, which is the accuracy typically sought/attained in such calculations.


As supplementary material to the accuracy discussion above, we tabulate the full numerical data underlying Figure~\ref{fig:ae-xc-accuracy-violin} in Tables~\ref{tab:ae-acc-lda-svwn-rep10}, \ref{tab:ae-acc-pbe-rep10}, \ref{tab:ae-acc-rscan-rep10}, \ref{tab:ae-hf-closed-subshell}, and~\ref{tab:ae-hf-charged-unpolarized} (LDA-SVWN, PBE, rSCAN, Hartree--Fock closed-shell, and Hartree--Fock charged species, respectively).
